...
import { Coffee, Mail, Lock, Eye, EyeOff, ArrowRight, User, CheckCircle2 } from 'lucide-react';
...
export function SignupView() {
  const { signup, setView, showToast } = useStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      showToast('Welcome to MoodBrew!', 'success');
      setView('home');
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        >
          ...
          <p className="text-white/60">Join the Pokhara coffee community</p>
        </motion.div>
          ...
          <div className="space-y-5">
                ...
                >
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm">
                      <CheckCircle2 className={`w-4 h-4 ${req.met ? 'text-green-400' : 'text-white/30'}`} />
            ...
            <div className="flex items-start gap-2">
              ...
              <Label htmlFor="terms" className="text-white/60 text-sm cursor-pointer leading-relaxed">
                I agree to the{' '}
                <button type="button" className="text-amber-400 hover:underline">Terms of Service</button>
                {' '}and{' '}
                <button type="button" className="text-amber-400 hover:underline">Privacy Policy</button>
              </Label>
            </div>
